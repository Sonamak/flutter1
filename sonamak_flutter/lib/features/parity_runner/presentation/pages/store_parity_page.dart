...
class StoreParityPage extends StatefulWidget { const StoreParityPage({super.key}); @override State<StoreParityPage> createState() => _StoreParityPageState(); }

class _StoreParityPageState extends State<StoreParityPage> {
  ...
  EndpointSpec(method: 'POST', path: '/store/change-status'),
  EndpointSpec(method: 'GET', path: '/store/critical'),
  EndpointSpec(method: 'POST', path: '/store/delete-sub-store/\${id}'),
  EndpointSpec(method: 'GET', path: '/store/excel-download'),
  EndpointSpec(method: 'GET', path: '/store/excel-expire'),
  EndpointSpec(method: 'POST', path: '/store/insert'),
  EndpointSpec(method: 'POST', path: '/store/insert-consumables'),
  EndpointSpec(method: 'POST', path: '/store/insert-sub-stores'),
  EndpointSpec(method: 'GET', path: '/store/item-usage'),
  EndpointSpec(method: 'POST', path: '/store/make-order'),
  EndpointSpec(method: 'POST', path: '/store/order-refund'),
  EndpointSpec(method: 'GET', path: '/store/orders'),
  EndpointSpec(method: 'GET', path: '/store/payment-excel'),
  EndpointSpec(method: 'GET', path: '/store/store-orders'),
  EndpointSpec(method: 'GET', path: '/store/sub-stores-tabs'),
  ];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Store & Inventory — Parity Runner')),
      body: Padding(
